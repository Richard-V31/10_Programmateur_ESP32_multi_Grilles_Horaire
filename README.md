...
Ce programme transforme un **ESP32** en programmateur horaire connecté, capable de piloter un nombre **configurable** de relais indépendants (4 par défaut, extensible sans toucher au code). Chaque relais peut fonctionner en mode **AUTOMATIQUE** (horaires programmés sur une grille de créneaux de 15 min) ou en mode **MANUEL** (forçage ON/OFF par l'utilisateur, depuis la page web ou un bouton poussoir physique).
...
- [Les plages horaires : la grille de créneaux](#les-plages-horaires--la-grille-de-créneaux)
...
## Les plages horaires : la grille de créneaux

### Principe

Chaque relais, en mode automatique, ne suit pas un simple couple "début → fin" mais une **grille couvrant les 24 heures**, découpée en créneaux de **15 minutes** (96 créneaux par jour). Chaque créneau est un bit : `1` = relais ON pendant ce quart d'heure, `0` = relais OFF.

Cela permet de définir **autant de plages que l'on veut** dans une même journée (aucune, une, dix...), y compris une plage **à cheval sur minuit** (ex: `22:45-06:15`) : ce cas particulier, source classique de bugs dans les programmateurs horaires, disparaît complètement puisque la grille est cyclique par nature.

La finesse des créneaux se règle en une seule ligne, en haut du fichier :

```cpp
const int SLOT_MIN = 15;   // 30 -> 48 créneaux | 15 -> 96 créneaux (défaut) | 10 -> 144 | 5 -> 288
```

`SLOT_MIN` doit être un diviseur de 1440 (5, 10, 15, 20, 30, 60...). Toute la chaîne — page web, stockage NVS, écran OLED, logique horaire — s'adapte automatiquement à cette valeur, sans autre modification.
...
Les horaires saisis sont automatiquement arrondis au créneau inférieur (avec `SLOT_MIN=15`, `09:05` devient `09:00`). Ces valeurs par défaut ne servent qu'au tout premier démarrage : dès qu'une grille a été enregistrée depuis la page web, c'est elle qui prévaut (voir [Sauvegarde des réglages](#sauvegarde-des-réglages-nvs)).

### Modifier les plages depuis la page web

Un appui sur les horaires affichés pour un relais ouvre une fenêtre d'édition : chaque ligne représente une heure, chaque case un créneau de 15 min qu'on coche/décoche au doigt. Des raccourcis sont disponibles : saisie rapide d'une plage via deux sélecteurs d'heure (+ boutons "ajouter"/"retirer"), "Tout effacer", "24h/24", "Inverser". La grille entière est envoyée à l'ESP32 en une seule requête, sous forme d'une chaîne hexadécimale compacte (24 caractères avec la résolution par défaut).

### Modifier les plages sans la page web (API)

La route `/save` accepte aussi un texte de plages, pratique en script ou en `curl` :

```bash
curl -X POST "http://richardv.local/save?id=1." -d "plages=06:30-08:00,18:45-22:30"
```

### Résumé lisible d'une grille

Le firmware sait reconstruire une description texte lisible à partir d'une grille de bits (utilisée à la fois par la page web et par l'écran OLED) : `"06:30-08:00, 11:30-13:15 +1"` par exemple, où `+1` indique qu'une plage supplémentaire existe mais n'est pas détaillée. Ce même mécanisme calcule aussi, à tout moment, le nombre de minutes restant avant le **prochain changement d'état** du relais (affiché sur la page web sous la forme "Extinction dans..."/"Allumage dans...").
...
- **Les horaires**, uniquement en mode automatique (rien n'est affiché ici en mode manuel, puisqu'aucun horaire ne s'applique) :
  ...
  - cas particuliers : `24h/24` (grille entièrement active) et `aucune plage` (grille vide).
...
Si l'heure n'est pas encore synchronisée par NTP au moment de l'affichage, l'écran retombe sur un résumé de secours : la première plage définie dans la grille, suivie de `+n` s'il y en a d'autres (ex: `06:30-08:00+2`).
...
| `/get-config` | GET | Décrit les relais à afficher (id, nom, couleur...) et la résolution de la grille (`SLOT_MIN`, `NB_SLOTS`). |
| `/get-data` | GET | État complet de tous les relais (grille, résumé, mode, état, minutes avant le prochain changement), interrogée chaque seconde. |
...
| `/save?id=...` | POST | Enregistre la nouvelle grille d'un relais (`grille=<hex>` ou `plages=<texte>`). |
...
Les grilles horaires, modes et états sont enregistrés dans la mémoire flash NVS (bibliothèque `Preferences`), namespace `config`, avec une clé par relais et par type d'information (ex: `1.gr` pour la grille, `1.auto` pour le mode, `1.etM` pour l'état) — ils survivent donc aux coupures de courant et aux redémarrages.
...
|---|---|---|
| `SLOT_MIN` | Durée d'un créneau de la grille horaire, en minutes | `15` |
